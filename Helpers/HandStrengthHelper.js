...
*****************************************************************/
    ...
    static getHandStrength(myHand, cardsOnBoard, nbrOpponents) {
        ...
        for (let iter = 1; iter <= config.HAND_STRENGTH_GAMES_NBR; iter++){
            let winner = true;
            let opponentsCards = this.giveOpponentsCards(nbrOpponents,actualDeck);
            const myHandValue = HandValueHelper.getHandValue(myHand.concat(cardsOnBoard));
            for (let cards of opponentsCards){
                let opponentHandValue = HandValueHelper.getHandValue(cards.concat(cardsOnBoard));
    ...
    static giveOpponentsCards(nbrOpponents,originalDeck){
        let actualDeck = originalDeck.slice(0);
        let opponentCards = [];
        for (let iterOpponent = 0;iterOpponent < nbrOpponents; iterOpponent++){
            opponentCards.push(CroupierHelper.getRandomCards(actualDeck, 2))
        }
